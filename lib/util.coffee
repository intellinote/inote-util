...
class Util
  ...
  #
  # Trims leading and trailing whitespace from a (possibly `null`) string.
  @trim:(str)=>str?.replace /(^\s+)|(\s+$)/ig, ""

  # **truncate** - *truncates a string to given width*
  #
  # A minimally "smart" truncation that looks for word boundaries.
  # The returned string will be AT MOST `width` characters wide.
  @truncate:(text,width)=>
    if not text? or not text.length? or text.length <= width
      ...
      while short_width > (width-max_shorten)
        if text.charAt(short_width) in break_chars
          return "#{text.substring(0,short_width)}…"
        else
          short_width--
      return "#{text.substring(0,width-1)}…"

  # **escape_for_json** - *escapes a string for use in a JSON string*
  #
  # Escapes the given string so that it can be inserted
  # into a JSON string.  For example, given
  # `He said, "I know."`, `escape_for_json` returns
  # `He said, \"I know.\"`.
  #
  # Note that the returned string is *not* wrapped in
  # quotation marks.
  @escape_for_json:(str)=>
    if str?
