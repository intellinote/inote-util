// Generated by CoffeeScript 1.12.6
(function() {
  var FileUtil, HOMEDIR, IMAGE_AS_DATA_URI, IOUtil, LIB_COV, LIB_DIR, fs, path, should;

  should = require('should');

  fs = require('fs');

  path = require('path');

  HOMEDIR = path.join(__dirname, '..');

  LIB_COV = path.join(HOMEDIR, 'lib-cov');

  LIB_DIR = fs.existsSync(LIB_COV) ? LIB_COV : path.join(HOMEDIR, 'lib');

  IOUtil = require(path.join(LIB_DIR, 'io-util')).IOUtil;

  FileUtil = require(path.join(LIB_DIR, 'file-util')).FileUtil;

  IMAGE_AS_DATA_URI = "data:image/png;base64,";

  describe('IOUtil', function() {
    it("can download URL content to a buffer", function(done) {
      this.timeout(5000);
      return IOUtil.download_to_buffer("https://www.intellinote.net/", (function(_this) {
        return function(err, buffer) {
          var str;
          should.not.exist(err);
          should.exist(buffer);
          Buffer.isBuffer(buffer).should.be.ok;
          buffer.length.should.not.be.below(1);
          str = buffer.toString();
          str.should.match(/<html/i);
          return done();
        };
      })(this));
    });
    it("can download URL content to a file", function(done) {
      var dest_file;
      this.timeout(5000);
      dest_file = path.join(HOMEDIR, 'test', 'IOUTIL-TEST-FILE.TXT');
      return IOUtil.download_to_file("https://www.intellinote.net/", dest_file, (function(_this) {
        return function(err) {
          var buffer, str;
          should.not.exist(err);
          buffer = fs.readFileSync(dest_file);
          should.exist(buffer);
          buffer.length.should.not.be.below(1);
          str = buffer.toString();
          str.should.match(/<html/i);
          FileUtil.rm(dest_file);
          return done();
        };
      })(this));
    });
    it("can pipe stream content to a buffer", function(done) {
      var in_stream, src_file;
      src_file = path.join(HOMEDIR, 'test', 'test-io-util.coffee');
      in_stream = fs.createReadStream(src_file);
      return IOUtil.pipe_to_buffer(in_stream, (function(_this) {
        return function(err, buffer) {
          var str;
          should.not.exist(err);
          should.exist(buffer);
          buffer.length.should.not.be.below(1);
          str = buffer.toString();
          str.should.match(/IOUtil/);
          str.should.match(/can pipe stream content to a buffer/);
          return done();
        };
      })(this));
    });
    it("can pipe stream content to a file", function(done) {
      var dest_file, in_stream, src_file;
      dest_file = path.join(HOMEDIR, 'test', 'IOUTIL-TEST-FILE.TXT');
      src_file = path.join(HOMEDIR, 'test', 'test-io-util.coffee');
      in_stream = fs.createReadStream(src_file);
      return IOUtil.pipe_to_file(in_stream, dest_file, (function(_this) {
        return function(err) {
          var buffer, str;
          should.not.exist(err);
          buffer = fs.readFileSync(dest_file);
          should.exist(buffer);
          buffer.length.should.not.be.below(1);
          str = buffer.toString();
          str.should.match(/IOUtil/);
          str.should.match(/can pipe stream content to a file/);
          FileUtil.rm(dest_file);
          return done();
        };
      })(this));
    });
    return it("can download URL content to data URI", function(done) {
      this.timeout(5000);
      return IOUtil.download_to_data_uri("https://api.intellinote.net/rest/img/intellinote-logo.png", (function(_this) {
        return function(err, uri) {
          should.not.exist(err);
          should.exist(uri);
          uri.should.equal(IMAGE_AS_DATA_URI);
          return done();
        };
      })(this));
    });
  });

}).call(this);

//# sourceMappingURL=test-io-util.js.map
