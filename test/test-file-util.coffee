...
describe 'FileUtil',->
  ...
  it "can make and remove directories",(done)->
    FileUtil.mkdir(path.join(HOMEDIR,"foo","bar","gamma"))
    fs.existsSync(path.join(HOMEDIR,"foo","bar","gamma")).should.be.ok
    FileUtil.rmdir(path.join(HOMEDIR,"foo","bar"))
    fs.existsSync(path.join(HOMEDIR,"foo","bar","gamma")).should.not.be.ok
    fs.existsSync(path.join(HOMEDIR,"foo","bar")).should.not.be.ok
    fs.existsSync(path.join(HOMEDIR,"foo")).should.be.ok
    FileUtil.rmdir(path.join(HOMEDIR,"foo"))
